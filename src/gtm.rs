...
pub fn default_plays() -> Vec<GtmPlay> {
    vec![
        GtmPlay {
            ...
            target_icp: "Operators with an outage-sensitive, location-specific decision and an existing operational workflow.".into(),
            target_vantages: vec!["process_owner".into(), "operator".into(), "technical_evaluator".into(), "router".into()],
            required_signal_keys: vec!["account.fit_evidence".into(), "account.outage_sensitive_decision".into(), "account.distributed_locations".into(), "account.existing_operational_system".into()],
